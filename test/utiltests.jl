...
@testset "Radial Integrals" begin
    a = 0.99
    met = Kang.Kerr(a)
    ...
    @testset "Case 4" begin
        ...
        @testset "It" begin
            ...
            probt = IntegralProblem(ft, rs, 1e5; nout=1)
            solt = solve(probt, HCubatureJL(); reltol=1e-7, abstol=1e-10)
            It = Kang.It_case4(met, roots, Kang._get_root_diffs(roots...), rs, τ4, λcase4)

            @test It ≈ solt.u + 1e7 + 2log(1e7) atol = 1e-3
        end
    end
end
