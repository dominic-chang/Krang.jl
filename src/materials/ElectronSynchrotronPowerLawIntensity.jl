...
@inline function synchrotronIntensity(
	...
	@allowscalar βv = βfluid[1]
	@allowscalar θz = βfluid[2]
	@allowscalar ϕz = βfluid[3]
	...
	@allowscalar magnetic_fieldx, magnetic_fieldy, magnetic_fieldz = magnetic_field
	@allowscalar p_fluid_ux = p_fluid_u[2]/ p_fluid_u[1]
	@allowscalar p_fluid_uy = p_fluid_u[3]/ p_fluid_u[1]
	@allowscalar p_fluid_uz = p_fluid_u[4]/ p_fluid_u[1]
	vec = [
...
"""
struct ElectronSynchrotronPowerLawIntensity{T, MT, βT, ST} <: AbstractMaterial
	magnetic_field::MT
	fluid_velocity::βT
	spectral_index::T
	R::T
...
"""
	function ElectronSynchrotronPowerLawIntensity(
		...
		mf = [magnetic_fieldx, magnetic_fieldy, magnetic_fieldz]
		βf = [fluid_speed, fluid_inclination_angle, fluid_azimuthal_angle]
		new{T, typeof(mf), typeof(βf), ST}(
			mf,
			βf,
			spectral_index,
			R,
...
"""
function (linpol::ElectronSynchrotronPowerLawIntensity)(
	pix::AbstractPixel{T},
	intersection,
) where {T}
